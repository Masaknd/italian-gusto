...
export async function generateMetadata({
  ...
  return {
    title: d.seo.reserveTitle,
    alternates: {
      canonical: `/${locale}/reserve`,
      languages: { ja: '/ja/reserve', en: '/en/reserve' },
    },
  };
}
export default async function ReservePage({
  ...
  const d = getDictionary(locale);
  return (
    <main className='mx-auto flex min-h-[65vh] w-[min(100%,var(--layout-container))] max-w-2xl flex-col justify-center px-8 py-20'>
      ...
      <p className='mt-6 leading-8 text-muted'>{d.reserve.body}</p>
      {siteConfig.reservationUrl ? (
        ...
        <p className='mt-8 rounded-card bg-surface-muted p-5'>
          {d.reserve.unavailable}{' '}
          <a
